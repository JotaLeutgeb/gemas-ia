...
- **Sistema visual (2026-08-22, v2 nocturno):** tema "boletín técnico de noche" — fondo tinta cálida `#171511`, texto crema `#ece7db`, filetes 1px `#37322a` con reglas fuertes `#a89f8d`, esquinas casi rectas, cero sombras/gradientes. Decisión del dueño: dark POR DEFECTO y sin toggle (CSP intacta, cero flash). Tipografía auto-hospedada: Fraunces Variable (display serif), IBM Plex Sans (cuerpo), IBM Plex Mono (todo dato numérico, tabular). Los gráficos usan tokens `THEME` de `src/scripts/charts.js` — jamás hex crueles en marks. **Excepción deliberada:** los PNG exportados para LinkedIn (`public/charts/`) se generan sobre papel claro porque el feed de LinkedIn es blanco; no "arreglar" eso. Nada de Google Fonts CDN (privacidad/GDPR). Cambiar la identidad visual = actualizar este párrafo + `global.css` + `THEME` + favicon.
